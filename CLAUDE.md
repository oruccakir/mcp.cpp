...
**Platform Abstraction Layer (PAL):** all OS access goes through `mcp::pal` (src/platform/pal.hpp) — descriptor I/O, `WakeEvent` (cross-thread poll unblocking), TCP, child processes, signal setup. Backends are selected at build time in src/CMakeLists.txt (`MCP_PAL_SOURCES`): posix/ (Linux + macOS) and win32/ (MSVC) are CI-verified; vxworks/ (VxWorks 7, RTP runtime-verified + DKM) is validated on an air-gapped toolchain via tools/pal_selftest — see docs/vxworks-port.md. `pal::process` is capability-gated (`MCP_PAL_HAS_PROCESS`; off on VxWorks — no fork/exec, so StdioClientTransport compiles out). **Never include OS headers (`<unistd.h>`, `<sys/*>`, `<poll.h>`, `<csignal>`, sockets, `<semLib.h>`/`<taskLib.h>`) outside src/platform/<backend>/** — the guard-rail is `grep -rln 'include <sys/\|include <unistd\|include <arpa\|include <netinet\|include <poll\|include <csignal\|include <semLib\|include <taskLib' src/` returning only platform backend files.

**Threading facade (`mcp::sys`, include/mcp/sys/threading.hpp):** the SDK never uses `std::thread`/`std::mutex`/`std::condition_variable`/`std::future` directly — those go through `mcp::sys::{thread,mutex,condition_variable}` + `mcp::sys::OneShot` (`std::lock_guard`/`std::unique_lock`/`std::atomic` stay std, they're OS-free). On host it's **pure `using`-aliases to std** (zero overhead; TSan exercises the real std types); on VxWorks (`MCP_SYS_VXWORKS_THREADS`) it's native taskSpawn+semaphores so kernel mode (no pthreads) works. When adding concurrency, use `mcp::sys` types, not `std::` ones.
